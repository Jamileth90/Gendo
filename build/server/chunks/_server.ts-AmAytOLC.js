...
const POST = async ({ request }) => {
  try {
    ...
    try {
      const sub = db.prepare("SELECT * FROM submitted_events WHERE id = ? AND status = ?").get(id, "pending");
      if (!sub) {
      ...
      let city = db.prepare("SELECT id FROM cities WHERE name LIKE ? LIMIT 1").get(`%${sub.city_name}%`);
      if (!city) {
        db.prepare(`
					INSERT INTO cities (name, country, country_code) VALUES (?, 'USA', 'US')
				`).run(sub.city_name);
        city = db.prepare("SELECT id FROM cities WHERE name = ?").get(sub.city_name);
      }
      let venueId = null;
      if (sub.venue_name) {
        let venue = db.prepare("SELECT id FROM venues WHERE city_id = ? AND name LIKE ? LIMIT 1").get(city.id, `%${sub.venue_name}%`);
        if (!venue) {
          db.prepare(`
						INSERT INTO venues (city_id, name, type, address) VALUES (?, ?, ?, ?)
					`).run(city.id, sub.venue_name, "other", sub.venue_address);
          venue = db.prepare("SELECT id FROM venues WHERE city_id = ? AND name = ?").get(city.id, sub.venue_name);
        }
...
export { POST };
//# sourceMappingURL=_server.ts-AmAytOLC.js.map
