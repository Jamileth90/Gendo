...
const POST = async ({ request }) => {
  try {
    ...
    const dateTs = dateStart ? Math.floor(new Date(dateStart).getTime() / 1e3) : Math.floor(Date.now() / 1e3);
    const dateEndTs = dateEnd ? Math.floor(new Date(dateEnd).getTime() / 1e3) : null;
    const db = new Database(DB_PATH);
    db.pragma("journal_mode = WAL");
    try {
      let resolvedCityName = cityName?.trim() || null;
      if (!resolvedCityName && cityId) {
        const c = db.prepare("SELECT name FROM cities WHERE id = ?").get(cityId);
...
export { POST };
//# sourceMappingURL=_server.ts-DpY-TsZh.js.map
