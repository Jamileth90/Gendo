...
async function runRefresh() {
  ...
  const before = db.prepare(`SELECT COUNT(*) as n FROM events WHERE status='active'`).get().n;
  db.close();
  ...
  const after = db2.prepare(`SELECT COUNT(*) as n FROM events WHERE status='active'`).get().n;
...
const POST = async () => {
  ...
  runRefresh().catch((e) => {
    ...
    lastRefreshLog.push(`❌ Error: ${e.message}`);
    refreshRunning = false;
  });
  return json({
    started: true,
...
export { GET, POST };
//# sourceMappingURL=_server.ts-uoY-Bz-u.js.map
