...
import { f as fix_stack_trace, s as set_private_env, a as set_public_env, p as public_env } from './chunks/shared-server-cF6ckHns.js';
import { c as create_ssr_component, s as setContext, v as validate_component, m as missing_component, n as noop, a as safe_not_equal } from './chunks/ssr-0v8FJN9_.js';
...
let relative$1 = (file) => file;
{
  try {
    const path = await import('node:path');
    const process = await import('node:process');
    relative$1 = (file) => path.relative(process.cwd(), file);
  } catch {
  }
}
function clean_up_stack_trace(error) {
  const stack_trace = (error.stack?.split("\n") ?? []).map((line) => {
    return line.replace(/\((.+)(:\d+:\d+)\)$/, (_, file, loc) => `(${relative$1(file)}${loc})`);
  });
