...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		// Find city
		const city = id
			? db.prepare(`SELECT * FROM cities WHERE id = ?`).get(id)
			: db.prepare(`SELECT * FROM cities WHERE name LIKE ? LIMIT 1`).get(`%${name}%`);
		...
		return json({ city, events, venues, typeCounts, total: (events as unknown[]).length });
	} finally {
		db.close();
	}
};
