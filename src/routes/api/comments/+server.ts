...
import { getSocialDb, getEventComments, getUserByToken } from '$lib/social/db';
	...
	try {
		...
		if (action === 'like') {
			...
			if (existing) {
				...
				const likes = (db.prepare(`SELECT COUNT(*) as n FROM comment_likes WHERE comment_id = ?`).get(commentId) as { n: number }).n;
