...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		// Eventos con venue: filtrar por v.lat, v.lng
		// Eventos sin venue: filtrar por c.lat, c.lng
		const events = db.prepare(`
			...
			LIMIT 200
		`).all(now, minLat, maxLat, minLng, maxLng, minLat, maxLat, minLng, maxLng) as Record<string, unknown>[];
