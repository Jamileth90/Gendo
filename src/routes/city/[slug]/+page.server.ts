...
export const load: PageServerLoad = async ({ params, cookies }) => {
	...
	const city = await get<{ id: number; name: string; country: string; countryCode: string; state: string | null; lat: number | null; lng: number | null }>(
		`SELECT id, name, country, country_code AS countryCode, state, lat, lng FROM cities WHERE name LIKE ? LIMIT 1`,
		`%${slug}%`
	);
	...
	const eventsRaw = await all<Record<string, unknown>>(`
	...
	const venues = await all<Record<string, unknown>>(`
	...
	const typeCounts = await all<{ type: string; count: number }>(`
	...
	const meetups = await getMeetups({ cityId: city.id, currentUserId: currentUser?.id, limit: 5 });
